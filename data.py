...
def interpolate_gaps(data, offset=0, num_data=1, resolution=5000, micro=True):
    ...
    dt_data = [[datetime_object(row[0], micro)] + row[1:] for row in cut_data] # changes dt string to dt object
    ...
    # the comprehension must be performed to flatten the list of lists from v_slice
    last = times[0]
    ...
    # 2. We have a list of gaps, now we have to use interp from np for each point.
    for start in gap_list[::-1]: # go backwards to avoid the indexes changing
        ...
        add_data = []
        for i in np.linspace(resolution, gap_size, num=gap_size / resolution - 1, endpoint=False): # iterate through each missing point
            dt_iter += timedelta(seconds=resolution) # goes res seconds ahead in time
            interpolated_data = [np.interp(i, xp, fps[j]) for j in range(num_data)]
            new_data = data[start][:offset] + [datetime_string(copy(dt_iter), micro)] + interpolated_data + data[start][offset + num_data + 1:]
            # above line takes the not data from start and just copies it
            add_data.append(new_data)
        data[start + 1 : start + 1] = add_data # shoves the additional data between start, start + 1
