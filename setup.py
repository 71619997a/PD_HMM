...
def setup_peak():
	...
	top_peaks = []
	raw_peaks = []
	for i, group in enumerate(eval_list[:-1]): # last freq set is cut off, kill it 
		...
		for dim in group[1]:
			...
			while len(all_peaks) < 3:
				all_peaks = detect_peaks(dim[10 : len(dim) / 2], mph=mph, mpd=10)
				mph -= 1
			index_fix = np.array(all_peaks) + 10
			sorted_peaks = sorted(index_fix, key=lambda index : -dim[index]) # - to sort fr top down
...
def setup_activities():
	...
	for row in l:
		...
		while activity in activity_windows: activity += 'i'
		activity_windows[activity] = [list(row) for row in np.rot90(data_window(o, start, end, 20))[:3].astype(float)] # remove date
	write_object('activity_dict', activity_windows)
	for act, data in activity_windows.iteritems():
		fft_activity[act] = [[mag(j) for j in fft(i)] for i in data]
	write_object('activity_dict_fft', fft_activity)

def setup_activity_peaks():
	a_dict = read_object('activity_dict_fft')
	out_dict_raw = {}
	out_dict_top3 = {}
	top_peaks = {}
	raw_peaks = {}
	for i, group in a_dict.iteritems(): 
		this_set = []
		raw_set = []
		for dim in group:
			all_peaks = []
			mph = 20
			while len(all_peaks) < 3:
				all_peaks = detect_peaks(dim[10 : len(dim) / 2], mph=mph, mpd=15)
				mph -= 1
			index_fix = np.array(all_peaks) + 10
			sorted_peaks = sorted(index_fix, key=lambda index : -dim[index]) # - to sort fr top down
			raw_set.append(sorted_peaks)
			top = sorted_peaks[:3]
			this_set.append(top)
		top_peaks[i] = this_set
		raw_peaks[i] = raw_set

	write_object('activity_peaks_raw', raw_peaks)
	write_object('activity_peaks_top3', top_peaks)

setup_activity_peaks()